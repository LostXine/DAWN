...
from .openx import OpenXDataset, as_gif, dataset2path
...
class DatasetIterator(torch.utils.data.IterableDataset):
    ...
    def __next__(self):
        ...
        if self.get_command:
            new_datum["command"] = str(datum["command"][0].numpy()).replace('_', ' ')
...
class OpenXTrajectoryDataset(OpenXDataset):
    def __init__(
...
if __name__ == "__main__":
    ...
    if False:
        ...
        display.Image(as_gif([x.resize((512, 512)) for x in image_list]))
