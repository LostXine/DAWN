...
class ImagineToAct(torch.nn.Module):
    ...
    def encode_text(self, text):
        text_batch = self.tokenizer(text, return_tensors="pt", padding="max_length", truncation=True, max_length=20).input_ids.to('cuda')
        text_cond = self.text_encoder(input_ids=text_batch).last_hidden_state
        return text_cond # [B, L, H]
