...
class TransformerDiffusionPolicy(nn.Module):
    def __init__(
        ...
        self.image_processor = AutoImageProcessor.from_pretrained('facebook/dinov3-convnext-small-pretrain-lvd1689m') #('facebook/dinov2-base')
        self.feature_extractor = AutoModel.from_pretrained('facebook/dinov3-convnext-small-pretrain-lvd1689m')
    ...
    def forward(
        ...
        visual_input = (x["visual_input"] * 255).to(torch.uint8)
        ...
        inp = self.image_processor(images=visual_input, return_tensors="pt").to(self.device)
        ...
        if "flow" in x:
            ...
            unnorm_flow = (x["flow"] * x["flow"].shape[-1]).float() # From [-1, 1]
            flow_rgb = flow_to_image(unnorm_flow).to(torch.uint8)
            flow_inp = self.image_processor(images=flow_rgb, return_tensors="pt").to(self.device)
            flow_feat = self.feature_extractor(**flow_inp).last_hidden_state  # [B, C, H, W] -> [B, N, C]
            visual_feat = torch.cat([visual_feat, flow_feat], dim=1)  # [B, N, C] -> [B, 2N, C]
