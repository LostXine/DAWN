...
class SDVideoMotion(nn.Module):
    def __init__(self, 
        ...
        # in_channels = 8
        self.in_channels = (num_frames + 1) * 4
        self.out_channels = (num_frames) * 4
        
        self.unet.register_to_config(in_channels=self.in_channels, sample_size=256 // 8, out_channels=self.out_channels)
        with torch.no_grad():
            new_conv_in = nn.Conv2d(
                self.in_channels, self.unet.conv_in.out_channels, self.unet.conv_in.kernel_size, self.unet.conv_in.stride, self.unet.conv_in.padding
            )
            new_conv_out = nn.Conv2d(
                self.unet.conv_out.in_channels, self.out_channels, self.unet.conv_out.kernel_size, self.unet.conv_out.stride, self.unet.conv_out.padding
            )
            for i in range(num_frames + 1):
                new_conv_in.weight[:, 4*i:4*(i+1), :, :].copy_(self.unet.conv_in.weight)
            for i in range(num_frames):
                new_conv_out.weight[4*i:4*(i+1), :, :, :].copy_(self.unet.conv_out.weight)
    ...
    def forward_eval(self, batch_data, inference_step=None, **kwargs):
        ...
        latents = torch.randn((b, self.in_channels - 4, h, w), device=image_latents.device)
        ...
        for t in self.noise_scheduler.timesteps:
            ...
            predicted_noise =  cond# 3 * cond - 2 * uncond
