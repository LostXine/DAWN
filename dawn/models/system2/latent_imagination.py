...
class LatentImagination(nn.Module):
    ...
    def forward(self, batch_data):
        ...
        goal_image = self.pipeline.image_processor.preprocess(batch_data["rgb_static"][:, 1])
        latents = self.vae.encode(goal_image).latent_dist.sample()
    ...
    def forward_eval(self, batch_data):
        ...
        image_goal = self.pipeline.image_processor.preprocess(batch_data["rgb_static"][:, 1])
        image = self.pipeline.image_processor.preprocess(batch_data["rgb_static"][:, 0])
        image_latents = self.vae.encode(image).latent_dist.sample()
        image_latents = image_latents * self.vae.config.scaling_factor  # Scale the latents
        ...
        # generated_goal = self.pipeline(
        #     prompt=text,
        #     image=batch_data["rgb_static"][:, 0], # If your pipeline takes a condition image
        #     num_inference_steps=25,
        #     # guidance_scale=7.5
        # ).images
        ...
        generated_goal = self.vae.decode(latents).sample
        generated_goal = (generated_goal / 2 + 0.5).clamp(0, 1)  # Scale back to [0, 1]
        ...
        with torch.no_grad():
            loss = torch.mean((generated_goal - image_goal) ** 2)
    ...
    def visualize(self, batch_data, outputs):
        ...
        gt_goal = batch_data["rgb_static"][:, 1]
        images = batch_data["rgb_static"][:, 0]
        text = batch_data["language"]

        generated_goal = outputs["generated_goal"]
        
        # Convert to numpy for visualization
        images_np = (images.permute(0, 2, 3, 1).cpu().numpy() * 255).astype(np.uint8)
        generated_goal_np = (generated_goal.permute(0, 2, 3, 1).cpu().numpy() * 255).astype(np.uint8)
        gt_goal_np = (gt_goal.permute(0, 2, 3, 1).cpu().numpy() * 255).astype(np.uint8)
        
        images = []
        for i in range(min(16, images_np.shape[0])):
            img = Image.fromarray(images_np[i])
            gt = Image.fromarray(gt_goal_np[i])
            generated = Image.fromarray(generated_goal_np[i])
