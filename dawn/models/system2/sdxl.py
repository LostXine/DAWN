...
class MotionSDXL(nn.Module):
    def __init__(self, 
        ...
        self.unet.up_blocks[0].requires_grad_(False)
        self.unet.down_blocks[0].requires_grad_(False)
    ...
    def forward(self, batch_data, **kwargs):
        ...
        # latents = torch.randn(bsz, 4, 32, 32, device=self.device, dtype=gt_rgb_flow.dtype)  # Random latents for training
    ...
    def forward_eval(self, batch_data, inference_step=None, **kwargs):
        ...
        for t in self.noise_scheduler.timesteps:
            ...
            predicted_noise =  3 * cond - 2 * uncond
